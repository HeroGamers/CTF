...
let a = Math.floor(Math.random() * 1000) * 43

function README() {
  let c = parseInt(document.getElementById("b").value)
  if (c === a) {
    fetch("/", {
        headers: {
          "X-Requested-With": "XMLHttpRequest"
      ...
      .then((d) => d.json())
      .then((e) => {
        document.querySelector("h1").textContent = "::" + e.flag
      })
      .catch((f) => {
        console.error("broken:", f)
      })
  } else {
    alert("checking.. flag will not be displayed if false")
  }
  a = Math.floor(Math.random() * 1000) * 43
}
document.getElementById("v").addEventListener("click", README)
